...
try{process.loadEnvFile(new URL('../../../.env',import.meta.url))}catch{/* .env is optional outside demo mode (e.g. Vercel, where secrets come from the dashboard) */}

/** Builds and configures the Nest app. Callers decide how to serve it: `app.listen()` for a long-running process, `app.init()` + delegate to the Express instance for a serverless request handler. */
export async function createApp() {
  const production = process.env.NODE_ENV === 'production';
  const app = await NestFactory.create(AppModule, { bufferLogs: true, logger: production ? undefined : ['error', 'warn'] });
  ...
  app.enableCors({
    ...
    allowedHeaders: ['content-type', 'authorization', 'x-demo-admin', 'x-claim-csrf', 'x-request-id'],
  });
  app.useGlobalPipes(new ValidationPipe({ whitelist: true, transform: true, exceptionFactory: (errors) => new BadRequestException({ code: 'VALIDATION_ERROR', message: 'Request validation failed', details: errors }) }));
  app.useGlobalFilters(new ApiExceptionFilter());
  const config = new DocumentBuilder().setTitle('Certificate Distribution API').setDescription('Secure Top 10 result, claim, export and retention APIs').setVersion('1.0').addBearerAuth().build();
